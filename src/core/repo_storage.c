...
static int ensure_minigit_dir(const char *base_path) {
    ...
    snprintf(path, sizeof(path), "%s/.minigit", base_path ? base_path : ".");
    if (MKDIR(path) != 0 && errno != EEXIST) return 0;
    
    snprintf(path, sizeof(path), "%s/.minigit/objects", base_path ? base_path : ".");
    if (MKDIR(path) != 0 && errno != EEXIST) return 0;
    
    snprintf(path, sizeof(path), "%s/.minigit/refs", base_path ? base_path : ".");
    if (MKDIR(path) != 0 && errno != EEXIST) return 0;
...
Repository *repo_load_from_dir(const char *path) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            int id = 0;
            if (sscanf(findData.cFileName, "%d.commit", &id) == 1) {
